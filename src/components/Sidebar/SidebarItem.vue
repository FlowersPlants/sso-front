  ...
  <div class="menu-wrapper">
    <template v-for="(item, index) in menu" v-if="!item.hidden">
      ...
      <el-submenu v-else :key="item.label" :index="filterPath(item)">
        ...
        <template slot="title" v-if="item.meta">
          <svg-icon v-if="item.meta.icon" :icon-class="item.meta.icon"></svg-icon>
          <span slot="title" v-if="item.meta.title" class="{'el-menu--display':isCollapse}">{{item.meta.title}}</span>
        </template>
        <template slot="title" v-else-if="item.children[0].meta">
          <svg-icon v-if="item.children[0].meta.icon" :icon-class="item.children[0].meta.icon"></svg-icon>
          <span slot="title" v-if="item.children[0].meta.title" :class="{'el-menu--display':isCollapse}">{{item.children[0].meta.title}}</span>
        </template>
        <!-- 递归添加子菜单 -->
        <template v-for="(child, cindex) in item.children">
          ...
          <sidebar-item v-else :menu="[child]" :key="cindex" :isCollapse="isCollapse" :pPath="parentPath(item.path)"></sidebar-item>
        </template>
      </el-submenu>
    </template>
  </div>
...
export default {
  ...
  methods: {
    ...
    open (item, child) {
      ...
      this.$router.push({
        path: resolveUrlPath(path, item.meta == null ? item.name : item.meta.title),
        query: item.query
      })
    }
  }
